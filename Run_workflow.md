...
6. After this is done, you can type python wf_ml_evaluation.py. (run the wf_ml_evaluation file). This is responsible for handling everything. It first splits the data into training and testing sets, after that wf_training is invoked which then trains on the training data and generates a model file. After that the wf_predication.py is called which then uses the model .pkl file as well as the test data in order to make predicitions (saved as predictions.csv in the evaluation folder). 
