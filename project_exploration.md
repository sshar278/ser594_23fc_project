...
Just run the wf_core.py file, it contains the subprocess to run both the wf_dataprocessing.py and then wf_visualization.py. A merged_data.db file will be generated which contains all the processed and cleaned data (this file can be viewed in the sqlite db browser). apart from that, the correlation.txt, summary.txt and the other processed data files get stored in the data_processed folder. And finally the visuals get stored inside the visuals folder. I will also be adding a video demo of mine showing the whole project run, where initially only the original data files, plus the .py files for core,dataprocessing and visualization are present and you can verify through the video that my code is actually producing those results that I will be providing.
